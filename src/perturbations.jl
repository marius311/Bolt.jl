...
@⌛ function boltsolve(hierarchy::Hierarchy)
    ...
    x₀ = first(bg.x_grid)
    u₀ = initial_conditions(x₀, hierarchy)
    prob = ODEProblem(hierarchy!, u₀, (x₀, zero(x₀)), hierarchy)
    sol = solve(prob, 𝕡.opts.boltsolve.alg; 𝕡.opts.boltsolve.reltol, saveat=hierarchy.bg.x_grid, dense=false)
    return sol
end


# make (Θ, Θᵖ, 𝒩) be zero-based indexing, since ComponentArrays
# components currently lose this information
function unpack(u)
    @unpack Θ, Θᵖ, 𝒩, Φ, δ, v, δ_b, v_b = u
    Θ  = OffsetArray(Θ,  -1)
    Θᵖ = OffsetArray(Θᵖ, -1)
    𝒩  = OffsetArray(𝒩,  -1)
    (;Θ, Θᵖ, 𝒩, Φ, δ, v, δ_b, v_b)
end
...
@⌛ function hierarchy!(du, u, hierarchy::Hierarchy{T, BasicNewtonian}, x) where T
    ...
    for ℓ in 2:(ℓmax_ν-1) #ℓmax_ν same as ℓmax_γ for massless nu for now
    ...
    du.Φ=Φ′; du.δ=δ′; du.v=v′; du.δ_b=δ_b′; du.v_b=v_b′
    return nothing
    
end

# BasicNewtonian Integrator (dispatches on hierarchy.integrator)
@⌛ function initial_conditions(x₀, hierarchy::Hierarchy{T, BasicNewtonian}) where T
    @unpack 𝕡, k, ℓmax_γ, ℓmax_ν, bg, ih = hierarchy
    ℋₓ, ℋₓ′, ηₓ, τₓ′, τₓ″ = bg.ℋ(x₀), bg.ℋ′(x₀), bg.η(x₀), ih.τ′(x₀), ih.τ″(x₀)
    H₀²,aᵢ² = bg.H₀^2,exp(x₀)^2
    Θ  = OffsetArray(zeros(T, ℓmax_γ+1), -1)
    Θᵖ = OffsetArray(zeros(T, ℓmax_γ+1), -1)
    𝒩  = OffsetArray(zeros(T, ℓmax_ν+1), -1)
    ...
    ComponentArray(; Θ, Θᵖ, 𝒩, Φ, δ, v, δ_b, v_b)
end
...
@⌛ function source_function(du, u, hierarchy::Hierarchy{T, BasicNewtonian}, x) where T
    ...
    @unpack k, ℓmax_γ, 𝕡, bg, ih = hierarchy
    H₀² = bg.H₀^2
    ...
    Θ,  Θᵖ,  𝒩,  Φ,  δ,  v,  δ_b,  v_b  = unpack(u)
    Θ′, Θᵖ′, 𝒩′, Φ′, δ′, v′, δ_b′, v_b′ = unpack(du)
